...
public class RobotContainer {
    ...
    private void configureBindings() {
        ...
        m_Controller2.a().whileTrue(new Rumble(m_Controller2) );
        

        m_mainController.rightTrigger(.8).onTrue(new TestShootSpeed(m_shooter,
                () -> SmartDashboard.getNumber("shooter/test_left_rpm", 0),
                () -> SmartDashboard.getNumber("shooter/test_right_rpm", 0)));

        m_mainController.x().onTrue(new Shoot(m_shooter,
                () -> {
        ...
        m_mainController.b().onTrue(new InstantCommand(m_driveTrain::lockWheels, m_driveTrain));
        m_mainController.a().onTrue(new InstantCommand(m_driveTrain::resetHeading, m_driveTrain));
        m_mainController.x().whileTrue(new StartEndCommand(m_driveTrain::togglePrecisionMode, m_driveTrain::togglePrecisionMode, m_driveTrain));
